...
function isValidProviderConfig(cfg) {
  ...
  if (provider !== PROVIDERS.OPENAI && provider !== PROVIDERS.HTTP) return false;
  if (provider === PROVIDERS.OPENAI && !String(cfg.apiKey || '').trim()) return false;
  if (provider === PROVIDERS.HTTP && !String(cfg.endpoint || '').trim()) return false;
  return true;
}

export function setProvider(userId, cfg, ttlMs = DAY_MS) {
  const id = String(userId || '').trim();
  if (!id) throw Object.assign(new Error('userId is required'), { status: 400 });
  if (!cfg || typeof cfg !== 'object') throw Object.assign(new Error('config object is required'), { status: 400 });
  const provider = String(cfg.provider || '').toLowerCase();
...
function purgeExpired() {
  ...
  for (const [id, entry] of store.entries()) {
    if (!entry || entry.expiresAt <= t) store.delete(id);
  }
}
