...
export async function requireAuth(req, res, next) {
  try {
    ...
    // Test bypass: no real Supabase needed for unit tests
        ...
        from() {
          return {
            ...
            select() {
              return {
                eq() {
                  return { single: async () => ({ data: null, error: { code: 'PGRST116', message: 'No rows' } }) };
                },
              };
            },
          };
    ...
    if (!url || !anon) return res.status(500).json({ ok: false, error: 'Server misconfigured: SUPABASE_URL/ANON_KEY required' });
